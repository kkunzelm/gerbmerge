...
Patched by KHK July 2015: 
        added support for Seeed Studio to generate an outline for all panelized gerber files
	the Seeeds Studio patch is copied from:   https://github.com/space-age-robotics/gerbmerge-patched/commit/5078c7b33439912c465fb33fc0082b82cfac3687#diff-0
	The patch adds the outlineLayers option to config file
...
def parseConfigFile(fname, Config=Config, Jobs=Jobs):
  ...
  # KHK patch begin: added support for Seeed Studio to generate an outline for all panelized gerber files
...
#        yspacing = 3
  ...
  for jobname in CP.sections():
    ...
    J = jobs.Job(jobname)  # KHK jobs = jobs.py, Job = class defined in jobs.py, jobname = Platinenname, mit mehreren Layern
