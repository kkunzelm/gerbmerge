...
def cluster(drills, tolerance, debug = _DEBUG):
    ...
    for size in sizes:
        ...
        # print "KHK drillcluster - drill size"
        # print size, round(size,1)
        
        # using metric drills it make no sense to use the maximum precision of 1 micrometer
        # it is more meaningful to round to 0.1 mm
        if config.Config['rounddrillsize'] == 1:
           size = round(size,1)    # KHK rounding drill size to 1/10 mm
